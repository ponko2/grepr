...
# Empty file
grep "foo" $DIR/empty.txt >"$OUT_DIR/foo.empty.txt" || true
...
grep "nobody" tests/inputs/nobody.txt >"$OUT_DIR/nobody.txt" || true
...
grep -c "nobody" tests/inputs/nobody.txt >"$OUT_DIR/nobody.txt.count" || true
